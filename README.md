# Regional Sales Summary

A small Python project that analyzes and summarizes fictional regional sales data. Built as part of Module 00-My first project of Coding Temple's AI Data Analytics Foundations course.

## What it does

- The script defines a dictionary of fictional sales values for five
  regions, then prints the total,average , min and Max sales of across all regions.
- Checks whether each region’s sales are above or below average using Boolean logic
- Writes the complete sales analysis into a text file

## How to run

You need Python 3.10 or higher. From the terminal, in this folder:

    python hello_northstar.py

(On Mac and Linux you may need `python3` instead of `python`.)

## Files

- `hello_northstar.py` - the main script
- `region_summary_report.txt` - text file containing the analysis output
- `hello_northstar_analysis.xlsx` - Excel workbook with sales data and analysis results
- `README.md` - this file, Project documentation and overview
